...
class Excel_import extends CI_Controller
{
	...
	function import_r7()
	{
		if(isset($_FILES["file_r7"]["name"]))
		{
			...
			$rs = $this->excel_import_model->insert($data);
			echo $rs ;
		}	
	}
		...
		if(isset($_FILES["file_thaiqm"]["name"]))
		{
			...
			foreach($object->getWorksheetIterator() as $worksheet)
			{
				...
				for($row=4; $row<=$highestRow; $row++)
				{
					...
					if($worksheet->getCellByColumnAndRow(2, $row)->getValue()=='ไม่เคย'){ $risk1=0;}else if($worksheet->getCellByColumnAndRow(2, $row)->getValue()=='เคย'){ $risk1=1;}
					if($worksheet->getCellByColumnAndRow(3, $row)->getValue()=='ไม่เคย'){ $risk2=0;}else if($worksheet->getCellByColumnAndRow(3, $row)->getValue()=='เคย'){ $risk2=1;}
					if($worksheet->getCellByColumnAndRow(4, $row)->getValue()=='ไม่เคย'){ $risk3=0;}else if($worksheet->getCellByColumnAndRow(4, $row)->getValue()=='เคย'){ $risk3=1;}
					$data[] = array(
						'd_update'		=>	date("Y-m-d H:i:s"),
						'cid'			=>	$cid,
						'name'			=>	$name,
